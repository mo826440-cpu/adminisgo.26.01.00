...
export default async function handler(req, res) {
  ...
  try {
    ...
    // URL base de la app: back_urls y webhook deben apuntar al dominio real
    const host = req.headers.host || ''
    const isProductionHost = host.includes('adminisgo.com')
    ...
    const appUrl = process.env.PUBLIC_APP_URL || process.env.VITE_PUBLIC_APP_URL ||
      (isProductionHost ? `https://${host}` : null) ||
      (isProductionEnv ? 'https://www.adminisgo.com' : null) ||
      (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://www.adminisgo.com')
