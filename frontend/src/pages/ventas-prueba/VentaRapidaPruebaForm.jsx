...
import '../ventas/VentasRapidas.css'
 ...
 * }} props
 */
const VentaRapidaPruebaForm = forwardRef(function VentaRapidaPruebaForm(
  { estadoCaja, onSuccess, onError, onSuccessMessage, onCancel },
  ref
) {
  ...
  const totalNumVista = totalNumeroNoNegativo(total)
  const totalPagosVista = filasPago.reduce((sum, row) => sum + montoCuentaParaCobro(row), 0)
  const pendienteVista = Math.max(0, totalNumVista - totalPagosVista)
  const diferenciaVista = totalPagosVista - totalNumVista

  return (
    <div className="formulario-venta-rapida-panel vp-vr-root">
      {edicionVenta ? (
        <div className="vp-vr-edicion-bar">
      ...
      {!loadingProducto && !productoVentaRapida ? (
        <div className="vp-vr-producto-faltante" role="alert">
          <span>
            No hay producto activo de venta rápida. Registralo para poder cargar ventas.
          </span>
          <Link
            ...
            title="Ir al formulario de producto"
          >
      ...
      <form id="venta-rapida-card-form" className="vp-vr-form" onSubmit={handleRegistrarVenta}>
        <div className="vp-vr-grid">
          <div className="vp-vr-col-left">
            <section className="vp-vr-card">
              <h2 className="vp-vr-card__title">1. Datos de la venta</h2>

              <div className="vp-vr-cliente-row">
                <label className="vp-vr-label" htmlFor="venta-rapida-cliente-search">
                  Cliente
                </label>
                  <div className="vp-vr-cliente-controls">
                  <div className="autocomplete-wrapper vp-vr-cliente-field">
                    <i className="bi bi-person vp-vr-cliente-field__icon" aria-hidden />
                    <input
                      id="venta-rapida-cliente-search"
                      name="venta_rapida_cliente_search"
                      ref={clienteInputRef}
                      type="text"
                      className="form-control vp-vr-input"
                  ...
                  <Link
                    to="/clientes"
                    className="vp-vr-icon-btn"
                    title="Abrir módulo Clientes"
                    aria-label="Abrir módulo Clientes"
                  >
              ...
              <label className="vp-vr-label" htmlFor="venta-rapida-total">
                Total de la venta
              </label>
              <input
                ...
                className="form-control vp-vr-input vp-vr-input--total"
                autoComplete="off"
                inputMode="decimal"
              ...
              <label className="vp-vr-label" htmlFor="venta-rapida-observaciones">
                Observaciones <span className="vp-vr-label__optional">(opcional)</span>
              </label>
              <input
                id="venta-rapida-observaciones"
                type="text"
                className="form-control vp-vr-input"
                value={observaciones}
                onChange={(e) => setObservaciones(e.target.value)}
                placeholder="Ej: Venta rápida de mostrador"
                autoComplete="off"
              />
            </section>

            <section className="vp-vr-card">
              <div className="vp-vr-card__head">
                <h2 className="vp-vr-card__title">2. Formas de pago</h2>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  className="vp-vr-add-pago"
              ...
              <div className="vp-vr-pagos" role="group" aria-label="Formas de pago">
                <div className="vp-vr-pagos__head">
                  <span>Forma de pago</span>
                  <span>Importe</span>
                  <span className="vp-vr-pagos__acciones-h">Acciones</span>
                </div>

                {filasPago.map((row, idx) => (
                  <div key={row.key} className="vp-vr-pagos__row">
                    <div className="vp-vr-pagos__metodo">
                      <i
                        className={`bi ${METODO_ICONOS[row.metodo_pago] || 'bi-wallet2'}`}
                        aria-hidden
                      />
                      <select
                    ...
                    <div className="vp-vr-pagos__importe">
                      <input
                    ...
                    <div className="vp-vr-pagos__acciones">
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                        className="vp-vr-trash"
              ...
              <div className="vp-vr-pagos__footer">
                <span>TOTAL PAGOS</span>
                <strong className="vp-vr-money vp-vr-money--ok">
                  {formatearMoneda(String(totalPagosVista))}
                </strong>
              </div>
            </section>

            <div className="vp-vr-info-banner">
              <i className="bi bi-info-circle-fill" aria-hidden />
              <p>
                Podés combinar múltiples formas de pago. Si el total de pagos es menor al total de
                la venta, la diferencia quedará como deuda pendiente.
              </p>
            </div>
          </div>

          <aside className="vp-vr-col-right">
            <section className="vp-vr-card vp-vr-resumen">
              <h2 className="vp-vr-card__title">3. Resumen de la venta</h2>
              <div className="vp-vr-resumen__hero">
                <span className="vp-vr-resumen__hero-label">Total de la venta</span>
                <div className="vp-vr-resumen__hero-value">
                  {formatearMoneda(String(totalNumVista))}
                </div>
              </div>
              <div className="vp-vr-resumen__rows">
                <div className="vp-vr-resumen__row">
                  <span>Total pagos</span>
                  <span>{formatearMoneda(String(totalPagosVista))}</span>
                </div>
                <div className="vp-vr-resumen__row">
                  <span>Pendiente</span>
                  <span className="vp-vr-money--warn">
                    {formatearMoneda(String(pendienteVista))}
                  </span>
                </div>
                <div className="vp-vr-resumen__row">
                  <span>Diferencia</span>
                  <span className="vp-vr-money--ok">
                    {formatearMoneda(String(diferenciaVista))}
                  </span>
                </div>
              </div>
            </section>

            <div className="vp-vr-hint-card">
              <div className="vp-vr-hint-card__icon" aria-hidden>
                <i className="bi bi-lightning-charge-fill" />
              </div>
              <div>
                <strong>Venta rápida</strong>
                <p>Se registrará como producto genérico. Sin detalle de productos.</p>
              </div>
            </div>

            <button
              type="submit"
              className="vp-vr-submit"
              disabled={
                saving ||
              ...
              title={
                saving || loadingProducto
                  ...
                  : !productoVentaRapida?.id
                    ...
                    : !estadoCaja?.cajaAbierta
                      ? 'Abrí la caja desde Gestión de caja para poder registrar ventas rápidas.'
                      : undefined
              }
            >
              {saving ? (
              ...
              <span className="vp-vr-submit__text">
                <strong>{edicionVenta ? 'GUARDAR CAMBIOS' : 'REGISTRAR VENTA'}</strong>
                <small>
                  {edicionVenta ? 'Confirmá para guardar la edición' : 'Confirmá para registrar la venta'}
                </small>
              </span>
            </button>

            {!estadoCaja?.cajaAbierta && !loadingProducto && productoVentaRapida?.id ? (
              <p className="vp-vr-aviso-caja" role="status">
                Abrí la caja desde el botón «Caja» del encabezado para poder registrar ventas.
              </p>
            ) : null}
            ...
            <Button
              type="button"
              variant="outline"
              className="vp-vr-cancel"
              onClick={() => {
                if (typeof onCancel === 'function') onCancel()
                else limpiarFormularioVentaRapida()
              }}
            >
...
export default VentaRapidaPruebaForm
