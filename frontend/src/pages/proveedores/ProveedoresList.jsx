          ...
          <div className="table-controls">
            ...
            {totalItems > 0 && (
              <div className="table-info">
                Mostrando {startIndex + 1}-{Math.min(endIndex, totalItems)} de {totalItems} proveedores
              </div>
            )}
          </div>
          ...
          {filteredProveedores.length === 0 ? (
            <div style={{ padding: '3rem', textAlign: 'center' }}>
              <p style={{ fontSize: '1.1rem', color: 'var(--text-secondary)' }}>
                {searchTerm ? 'No se encontraron proveedores con ese criterio de búsqueda.' : 'No hay proveedores registrados aún.'}
              </p>
              {!searchTerm && (
                <Link to="/proveedores/nuevo">
                ...
                <table className="table table-sticky-header">
                  ...
                  <tbody>
                    {paginatedProveedores.map(proveedor => (
                      <tr key={proveedor.id}>
                        <td><strong>{proveedor.nombre_razon_social}</strong></td>
                        <td>{proveedor.email || '-'}</td>
                        <td>{proveedor.telefono || '-'}</td>
                        <td>{proveedor.cuit_rut || '-'}</td>
